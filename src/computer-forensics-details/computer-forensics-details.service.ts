// Arquivo: src/computer-forensics-details/computer-forensics-details.service.ts
...
@Injectable()
export class ComputerForensicsDetailsService {
  constructor(
    @InjectRepository(ComputerForensicsDetail)
    private detailsRepository: Repository<ComputerForensicsDetail>,
  ...
  async create(createDto: CreateComputerForensicsDetailDto, currentUser: User): Promise<ComputerForensicsDetail> {
    ...
    if (existingDetails) {
      throw new ConflictException(`Detalhes de perícia de informática já existem para esta ocorrência.`);
    }
  ...
  async findByOccurrenceId(occurrenceId: string): Promise<ComputerForensicsDetail> {
    const details = await this.detailsRepository.findOne({
      where: { occurrence: { id: occurrenceId } },
      relations: ['examsPerformed'],
    });
    if (!details) {
      throw new NotFoundException(`Nenhum detalhe de perícia de informática encontrado para a ocorrência com ID "${occurrenceId}".`);
    }
    return details;
  }

  async update(id: string, updateDto: UpdateComputerForensicsDetailDto, currentUser: User): Promise<ComputerForensicsDetail> {
    const details = await this.detailsRepository.findOne({
      where: { id },
    ...
    if (!details) {
      throw new NotFoundException(`Detalhes de perícia de informática com o ID "${id}" não encontrados.`);
    }
