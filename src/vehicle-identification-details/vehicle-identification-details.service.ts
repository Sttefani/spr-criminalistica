// Arquivo: src/vehicle-identification-details/vehicle-identification-details.service.ts
...
@Injectable()
export class VehicleIdentificationDetailsService {
  constructor(
    @InjectRepository(VehicleIdentificationDetail)
    private detailsRepository: Repository<VehicleIdentificationDetail>,
  ...
  async create(createDto: CreateVehicleIdentificationDetailDto, currentUser: User): Promise<VehicleIdentificationDetail> {
    ...
    if (existingDetails) {
      throw new ConflictException(`Detalhes de identificação veicular já existem para esta ocorrência.`);
    }
  ...
  async findByOccurrenceId(occurrenceId: string): Promise<VehicleIdentificationDetail> {
    const details = await this.detailsRepository.findOne({
      where: { occurrence: { id: occurrenceId } },
      relations: ['examsPerformed'],
    });
    if (!details) {
      throw new NotFoundException(`Nenhum detalhe de identificação veicular encontrado para a ocorrência com ID "${occurrenceId}".`);
    }
    return details;
  }

  async update(id: string, updateDto: UpdateVehicleIdentificationDetailDto, currentUser: User): Promise<VehicleIdentificationDetail> {
    const details = await this.detailsRepository.findOne({
      where: { id },
    ...
    if (!details) {
      throw new NotFoundException(`Detalhes de identificação veicular com o ID "${id}" não encontrados.`);
    }
