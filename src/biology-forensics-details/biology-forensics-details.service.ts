// Arquivo: src/biology-forensics-details/biology-forensics-details.service.ts
...
@Injectable()
export class BiologyForensicsDetailsService {
  constructor(
    @InjectRepository(BiologyForensicsDetail)
    private detailsRepository: Repository<BiologyForensicsDetail>,
  ...
  async create(createDto: CreateBiologyForensicsDetailDto, currentUser: User): Promise<BiologyForensicsDetail> {
    ...
    if (existingDetails) {
      throw new ConflictException(`Detalhes de biologia forense já existem para esta ocorrência.`);
    }
  ...
  async findByOccurrenceId(occurrenceId: string): Promise<BiologyForensicsDetail> {
    const details = await this.detailsRepository.findOne({
      where: { occurrence: { id: occurrenceId } },
      relations: ['examsPerformed'],
    });
    if (!details) {
      throw new NotFoundException(`Nenhum detalhe de biologia forense encontrado para a ocorrência com ID "${occurrenceId}".`);
    }
    return details;
  }

  async update(id: string, updateDto: UpdateBiologyForensicsDetailDto, currentUser: User): Promise<BiologyForensicsDetail> {
    const details = await this.detailsRepository.findOne({
      where: { id },
    ...
    if (!details) {
      throw new NotFoundException(`Detalhes de biologia forense com o ID "${id}" não encontrados.`);
    }
