// Arquivo: src/ballistics-details/ballistics-details.service.ts
...
@Injectable()
export class BallisticsDetailsService {
  constructor(
    @InjectRepository(BallisticsDetail)
    private detailsRepository: Repository<BallisticsDetail>,
  ...
  async create(createDto: CreateBallisticsDetailDto, currentUser: User): Promise<BallisticsDetail> {
    ...
    if (existingDetails) {
      throw new ConflictException(`Detalhes de balística já existem para esta ocorrência.`);
    }
  ...
  async findByOccurrenceId(occurrenceId: string): Promise<BallisticsDetail> {
    const details = await this.detailsRepository.findOne({
      where: { occurrence: { id: occurrenceId } },
      relations: ['examsPerformed'],
    });
    if (!details) {
      throw new NotFoundException(`Nenhum detalhe de balística encontrado para a ocorrência com ID "${occurrenceId}".`);
    }
    return details;
  }

  async update(id: string, updateDto: UpdateBallisticsDetailDto, currentUser: User): Promise<BallisticsDetail> {
    const details = await this.detailsRepository.findOne({
      where: { id },
    ...
    if (!details) {
      throw new NotFoundException(`Detalhes de balística com o ID "${id}" não encontrados.`);
    }
